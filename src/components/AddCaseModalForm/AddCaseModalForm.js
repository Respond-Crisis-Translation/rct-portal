...
class AddCaseModalForm extends React.Component {
  ...
  handleDocDescription = (e) => this.setState({docDescription: e.target.value})
  handleSensitiveContentPresent = () => this.setState({sensitiveContents: true})
  handleSensitiveContentAbsent = () => this.setState({sensitiveContents: false})
    ...
    return(
      ...
      <Modal.Body>
          <Form.Group >
              ...
              <Form.Control type="text" onChange={this.handleFirstname} placeholder=""/>           
          </Form.Group>
          <Form.Group >
              <Form.Label>Client's last name</Form.Label>
              <Form.Control type="text" onChange={this.handleLastname} placeholder=""/>           
          </Form.Group>
          <Form.Group >
              <Form.Label>Case due date</Form.Label>
              <Form.Control type="date" onChange={this.handleDuedate} placeholder=""/>           
          </Form.Group>
          <Form.Group >
              <Form.Label>Organization</Form.Label>
              <Form.Control type="text" onChange={this.handleOrganization} placeholder=""/>           
          </Form.Group>
          <Form.Group >
              <Form.Label>Point of contact at organization</Form.Label>
              <Form.Control type="text" onChange={this.handlePocName} placeholder=""/>           
          </Form.Group>
          <Form.Group >
              <Form.Label>Point of contact info</Form.Label>
              <Form.Control type="email" onChange={this.handlePocInfo} placeholder=""/>           
          </Form.Group>
          <Form.Group >
              <Form.Label>Page count</Form.Label>
              <Form.Control type="number" onChange={this.handlePageCount} placeholder="" min="0"/>           
          </Form.Group>
          <Form.Group>
              <Form.Label>Document description</Form.Label>
              <Form.Control as="textarea" rows={3} onChange={this.handleDocDescription} placeholder=""/>
          </Form.Group>
          <Form.Group>
            Sensitive contents (including graphic description, images, etc.)
            <div key={`inline-radio`} className="mb-3">
              <Form.Check type="radio" label="Yes" name="formRadios" id={`inline-radio-1`} onChange={this.handleSensitiveContentPresent}/>
              <Form.Check type="radio" label="No" name="formRadios" id={`inline-radio-2`} onChange={this.handleSensitiveContentAbsent}/>
            </div>
          </Form.Group>
          <Form.Group>
